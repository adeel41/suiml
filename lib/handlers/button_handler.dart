...
class ButtonHandler extends Handler {
	...
	_handleAnimatedButtons( Document document ) {
		doForEach( document, "button-animated", ( element ) {
			...
			if ( element.attributes.containsKey( "animation" ) ) {
				...
				if ( animationType.toLowerCase( ) == "vertical" )classes.addAll( ["vertical", "animated"] );
				else if ( animationType.toLowerCase( ) == "fade" )classes.addAll( ["animated", "fade"] );

				//remove animation as it was only for the transformer.
				element.attributes.remove( "animation" );
			}
			elseclasses.add( "animated" );
	...
	_handleLabeledButtons( Document document ) {
		doForEach( document, "button-labeled", ( element ) {
			new ElementBuilder( "div" )
				..addClasses( ["ui", "labeled", "button"] )
	...
	void handle( Document document ) {
		...
		_handleLabeledButtons( document );
	}
}
